...
class PlayStatusCallBackTask: public ICallBackTask{
private:
    std::wstring m_name_space;
public:
    PlayStatusCallBackTask(const std::wstring &name_space):
        ICallBackTask(L"PlayStatusCallBackTask"),
        m_name_space(name_space)
    {
    }
    bool AddCallBack(Measure *measure) override{
        if(measure->play_action.empty()){
            return false;
        }
        switch(measure->type){
